...
namespace ChuChu.Framework.SceneAndObjectManagement
{
    ...
    {
        [Header("Assign Scene")]
        [Tooltip("Assign specific Scene to load.")]public ChuChuScene ID; 

        [Header("Configurations")]
        [Tooltip("Assign specific UIConfig to load.")] public UIConfig UIConfig; //UI物件在讀取後就被放置在指定的Canvas (佔不考慮未來是否UI亂飛會換Canvas)
    ...
    {
        public IManageableObject Prefab;
        public int Amount = 1; //todo Editor防呆
    ...
    {
        internal ObjectFlag ObjectFlag = ObjectFlag.Other;
